...
public class CurrentUserWorkloadServlet extends HttpServlet {
    ...
    protected void doGet(final HttpServletRequest req, final HttpServletResponse resp) throws ServletException, IOException {
        ...
        while (it.hasNext()) {
            ...
            if (obj instanceof IProject) {
                ...
                IPObjectList items = trackerService.queryWorkItems(prj,"assignee.id:" + currentUser,"remainingEstimate");
                if (items.size() > 0) {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {   
        	...
        	ITrackerService trackerService=(ITrackerService)PlatformContext.getPlatform().lookupService(ITrackerService.class);
        	ITrackerProject project=trackerService.getTrackerProject("elibrary");
        	if(project==null) {
        		throw new Exception("project not found.");
        	}
        	List<ITypeOpt> workItemTypes=project.getWorkItemTypeEnum().getAllOptions();
        	req.setAttribute("workItemTypes", workItemTypes);
        	String title=req.getParameter("title");
        	String type=req.getParameter("type");
        	
        	if(title !=null&&type!=null&&!title.isEmpty()&&!type.isEmpty()) {
        		IWorkItem newWorkItem=project.createWorkItem(type);
        		newWorkItem.setTitle(title);
        		newWorkItem.setCustomField("name", "hakeem");
        		newWorkItem.save();
        		req.setAttribute("createdWorkItem", newWorkItem);
        	}
